...
public class ThreeServerQueue extends JPanel {
    ...
    public ThreeServerQueue (boolean doAnimation, int k, double a)
    {
    ...
    {
	return exponential (serviceRates);
    }
	...
	if ( (argv == null) || (argv.length != 2) ) {
	    System.out.println ("Usage: java ThreeServerQueue 2 1.6");
	    System.exit(0);
	}
    ...
    if (k<=0){
        k=1;//at least one queue
    }
    if (a<=0){
        a=0.1;//at least 0.1
    }

    ThreeServerQueue q = new ThreeServerQueue (false,k,a);
    q.reset ();
    while (true) {
    q.nextStep ();
    }
