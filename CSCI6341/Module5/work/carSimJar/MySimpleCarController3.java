...
public class MySimpleCarController3 implements CarController {
    ...
    public void move ()
    {
        ...
        double distance = sPack.dist(sPack.getX(),sPack.getY(),endX,endY);
        double goalAngle = Math.atan2(endY-sPack.getY(),endX-sPack.getX());
        if (status == 0){
            vel = 2;
            phi = 10*(goalAngle-sPack.getTheta());
            if (goalAngle-sPack.getTheta() < 0.1){
                status = 1;
            }
        }else{
            if (dNE > 100){
            ...
            if (openToGoal(goalAngle,distance,sPack.getTheta(),sPack.sonarDistances)){
                status = 0;
            }
        }
