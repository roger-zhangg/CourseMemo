...
public class QueueControl extends JPanel {
    ...
    // Statistics.
    int numCustomers = 0;                   // How many arrived?
    int numDeparted;                        // How many left?
	...
	if (numDeparted % 1000 == 0) {
	    System.out.println ("After " + numDeparted + " departures: avgWait=" + avgWaitTime + "  avgSystemTime=" + avgSystemTime);
	}
    }
	...
	numCustomers ++;
	int k = chooseQueue ();
	queues[k].add (new Customer (currentTime));
	if (queues[k].size() == 1) {
	...
	Customer c = queues[k].removeFirst ();
        ...
        double timeInSystem = currentTime - c.entryTime;
	...
	if (queues[k].size() > 0) {
	    ...
	    Customer waitingCust = queues[k].get (0);
            ...
            double waitTime = currentTime - waitingCust.entryTime; 
	...
	double nextArrivalTime = currentTime + randomInterarrivalTime();
    ...
    {
	double nextDepartureTime = currentTime + randomServiceTime (i);
    ...
    {
	return exponential (serviceRate[i]);
    }
	...
	avgWaitTime = totalWaitTime / numDeparted;
	avgSystemTime = totalSystemTime / numDeparted;
    }
	...
	// Current averages.
	String str = "numDeparted=" + numDeparted + "  Avg wait = " + df.format(avgWaitTime) + "   avg time in system = " + df.format(avgSystemTime);
